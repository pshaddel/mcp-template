...
describe("Weather Tool Tests", () => {
	it("tool should have a truthy string as name", () => {
		assert.ok(
			typeof weatherTool.tool_name === "string" &&
				weatherTool.tool_name.length > 0,
		);
	});

	it("tool should have a truthy string as description", () => {
		assert.ok(
			typeof weatherTool.description === "string" &&
				weatherTool.description.length > 0,
		);
	});

	it("Should get error when not passing required parameters", async () => {
		assert.ok(
			(await weatherTool.function({})).content[0].text.includes(
				"Failed to fetch weather data",
			),
		);
		assert.ok(
			(
				await weatherTool.function({ latitude: "47.8095" })
			).content[0].text.includes("Failed to fetch weather data"),
		);
		assert.ok(
			(
				await weatherTool.function({ longitude: "13.0550" })
			).content[0].text.includes("Failed to fetch weather data"),
		);
		assert.ok(
			(
				await weatherTool.function({ start_date: "2025-07-25" })
			).content[0].text.includes("Failed to fetch weather data"),
		);
		assert.ok(
			(
				await weatherTool.function({ end_date: "2025-07-25" })
			).content[0].text.includes("Failed to fetch weather data"),
		);
	});
